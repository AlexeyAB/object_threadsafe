...
template<typename T>
void func(safe_ptr<T> safe_map_strings) {
    for (size_t i = 0; i < 10000; ++i) {
        safe_map_strings->at("apple")++;
        safe_map_strings->find("potato")->second++;
    }
}

int main() {    
    safe_ptr< std::map<std::string, int> > safe_map_strings;    
    (*safe_map_strings)["apple"] = 0;
    (*safe_map_strings)["potato"] = 0;

    std::thread t1([&]() { func(safe_map_strings);} ); 
    std::thread t2([&]() { func(safe_map_strings);} ); 
    t1.join(); t2.join();
    
    auto const readonly_safe_map_string = safe_map_strings;
    std::cout << "potato is " << readonly_safe_map_string->at("potato") <<
