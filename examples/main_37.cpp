...
template<typename T>
void benchmark_safe_ptr(T safe_map, size_t const iterations_count, size_t const container_size)
{
...
int main() {
    ...
    std::cout << "safe<map,contf>tablock: \n"; 
    for (auto &i : vec_thread) i = std::move(std::thread([&]() {
        benchmark_safe_ptr(safe_map_contfree_global, iterations_count, container_size);
    }));
